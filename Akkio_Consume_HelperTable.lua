...
local function MigrateSettings()
    ...
    if savedVersion ~= currentVersion then
        DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Akkio Consume Helper:|r Migrating settings from v" .. savedVersion ..
                                          " to v" .. currentVersion)
        ...
        if savedVersion == "0.0.0" then
            ...
            if not Akkio_Consume_Helper_Settings.settings or type(Akkio_Consume_Helper_Settings.settings) ~= "table" then
                DEFAULT_CHAT_FRAME:AddMessage(
                    "|cffFF6B6BWarning:|r Old settings detected. Resetting to defaults for compatibility.")
...
local function updateBuffTracker(buffName, buffTexture, forceReset)
    if not buffName or not buffTexture then
        return
    end
    ...
    if not buffTracker[buffName] or forceReset then
                    ...
                    if itemLink then
                        ...
                        if linkItemName then
                            ...
                            if linkItemName and itemCount then
                                ...
                                buffStatusFrame.bagCache[linkItemName] =
                                    (buffStatusFrame.bagCache[linkItemName] or 0) + actualCount
                            end
                        end
                    end
                end
...
local function UpdateBuffStatusOnly()
    ...
    if Akkio_Consume_Helper_Settings.settings.pauseUpdatesInCombat and Akkio_Consume_Helper_Settings.settings.inCombat then
        return
    end
    ...
    for i = 1, table.getn(buffStatusFrame.children) do
        ...
        if icon and icon.buffdata then
            ...
            elseif data.isWeaponEnchant then
                ...
                local activeTexture = (activeBuffs[data.buffIcon] and data.buffIcon) or
                                          (activeBuffs[data.raidbuffIcon] and data.raidbuffIcon)
            ...
            if icon.amountLabel then
                local itemAmount = data.isWeaponEnchant and findItemChargesInBag(data.name) or
                                       findItemInBagAndGetAmount(data.name)
...
BuildSettingsUI = function(panel)
    ...
    local splitByCategoryCheckbox = CreateFrame("CheckButton", "AkkioSplitByCategoryCheckbox", settingsFrame,
    ...
    local pauseUpdatesCheckbox = CreateFrame("CheckButton", "AkkioPauseUpdatesCheckbox", settingsFrame,
        "UICheckButtonTemplate")
    ...
    local hideFrameCheckbox = CreateFrame("CheckButton", "AkkioHideFrameCheckbox", settingsFrame,
        "UICheckButtonTemplate")
    ...
    local showTooltipsCheckbox = CreateFrame("CheckButton", "AkkioShowTooltipsCheckbox", settingsFrame,
        "UICheckButtonTemplate")
    ...
    local hoverToShowCheckbox = CreateFrame("CheckButton", "AkkioHoverToShowCheckbox", settingsFrame,
        "UICheckButtonTemplate")
    ...
    -- Lock Frame Checkbox
    local lockFrameCheckbox = CreateFrame("CheckButton", "AkkioLockFrameCheckbox", settingsFrame,
        "UICheckButtonTemplate")
    ...
    applyButton:SetScript("OnClick", function()
        ...
        if buffStatusFrame then
            ...
            if lockFrameValue then
                ...
                if buffStatusFrame.bg then
                    buffStatusFrame.bg:Hide()
                end
                if buffStatusFrame.titleBar then
                    buffStatusFrame.titleBar:Hide()
                end
                if buffStatusFrame.title then
                    buffStatusFrame.title:Hide()
                end
            else
                buffStatusFrame:SetMovable(true)
                buffStatusFrame:RegisterForDrag("LeftButton")
                if buffStatusFrame.bg then
                    buffStatusFrame.bg:Show()
                end
                if buffStatusFrame.titleBar then
                    buffStatusFrame.titleBar:Show()
                end
                if buffStatusFrame.title then
                    buffStatusFrame.title:Show()
                end
            end
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Scale:|r " .. tostring(scaleValue) .. " |cffFFFF00Timer:|r " ..
                                          timerValue .. "s |cffFFFF00Icons per row:|r " .. iconsPerRowValue ..
                                          " |cffFFFF00Icon spacing:|r " .. iconSpacingValue .. "px")
    end)
...
BuildResetConfirmationUI = function()
    ...
    resetConfirmFrame:SetScript("OnDragStart", function()
        resetConfirmFrame:StartMoving()
    end)
    resetConfirmFrame:SetScript("OnDragStop", function()
        resetConfirmFrame:StopMovingOrSizing()
    end)
...
BuildBuffSelectionUI = function(panel)
    ...
    buffSelectFrame.scrollbar = CreateFrame("Slider", "AkkioBuffScrollBar", buffSelectFrame.scrollframe,
...
BuildBuffStatusUI = function()
    ...
    if not buffStatusFrame then
        ...
        buffStatusFrame:RegisterForDrag("LeftButton")
        buffStatusFrame:SetScript("OnDragStart", function()
            buffStatusFrame:StartMoving()
        end)
    ...
    for _, data in ipairs(enabledBuffsList) do
        ...
        if table.getn(iconPool) > 0 then
            ...
            if icon.timerLabel then
                icon.timerLabel:Hide()
            end
            if icon.slotIndicator then
                icon.slotIndicator:Hide()
            end
        else
            isNewIcon = true
            ...
            -- add zoom to remove ugly border
        ...
        if isNewIcon then
            icon:SetScript("OnClick", function()
                ...
                if buffdata.isWeaponEnchant then
                    ...
                    --     DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Info:|r Your " .. buffdata.slot ..
                    --                                       " weapon already has an enchant applied.")
                    -- else
                ...
                if buffdata.canBeAnounced == false and findItemInBagAndGetAmount(buffdata.name) > 0 then
                    local used = findAndUseItemByName(buffdata.name)
                    if used then
                        updateBuffTracker(buffdata.name, buffdata.buffIcon or buffdata.raidbuffIcon, true)
                        ForceRefreshBuffStatus()
                    end
                end
                -- end
                    ...
                    if tooltipHasBuff then
                        ...
                        if buffdata.slot == "mainhand" then
                            ...
                            if hasMainHandEnchant and mainHandExpiration then
                                ...
                                GameTooltip:AddLine("Time remaining: " .. formatTimeRemaining(remainingSeconds), 1, 1,
                                    0, 1)
                            end
...
SlashCmdList["AKKIODEBUG"] = function()
    ...
    if hasMainHandEnchant then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cffFFFFFFMain Hand:|r |cff00FF00Enchanted|r (" ..
    ...
    if hasOffHandEnchant then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cffFFFFFFOff Hand:|r |cff00FF00Enchanted|r (" ..
                                          formatTimeRemaining(remainingSeconds) .. " remaining)")
    else
    ...
    if buffStatusFrame then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Hover-to-show enabled:|r " ..
                                          (hoverEnabled and "|cff00FF00YES|r" or "|cffFF6B6BNO|r"))
        ...
        if hoverEnabled then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Hide Timer:|r " ..
                                              (hideTimer and string.format("%.2f", hideTimer) or "nil"))
            ...
            if hideTimer then
                ...
                DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Time until hide:|r " .. string.format("%.2f", timeUntilHide) ..
                                                  "s")
            end
            ...
            DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00State Match:|r " ..
                                              (stateMatch and "|cff00FF00YES|r" or "|cffFF6B6BMISMATCH!|r"))

            -- Check if frame is actually shown/hidden
            local frameShown = buffStatusFrame:IsShown()
            DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Frame IsShown:|r " ..
                                              (frameShown and "|cff00FF00YES|r" or "|cffFF6B6BNO|r"))
            ...
            if not stateMatch then
                ...
                if currentAlpha > 0 and (not hoverCount or hoverCount <= 0) then
                    DEFAULT_CHAT_FRAME:AddMessage(
                        "|cffFF6B6BFrame stuck visible!|r Hover count suggests it should be hidden.")
                    -- Check for expired timer specifically
                    if hideTimer and currentTime >= hideTimer then
                        local expiredTime = currentTime - hideTimer
                        DEFAULT_CHAT_FRAME:AddMessage("|cffFF6B6BHide timer expired " ..
                                                          string.format("%.2f", expiredTime) ..
                                                          "s ago but frame is still visible!|r")
                    end
                ...
                elseif currentAlpha == 0 and hoverCount and hoverCount > 0 then
                    DEFAULT_CHAT_FRAME:AddMessage(
                        "|cffFF6B6BFrame stuck hidden!|r Hover count suggests it should be visible.")
                    DEFAULT_CHAT_FRAME:AddMessage("|cffFFFF00Suggestion:|r Try /acthoverfix to reset hover state")
                end
            end
        end
...
CreateWelcomeWindow = function()
    ...
    welcome:SetScript("OnDragStart", function()
        welcome:StartMoving()
    end)
    welcome:SetScript("OnDragStop", function()
        welcome:StopMovingOrSizing()
    end)
    ...
    settingsDesc:SetText(
        "|cffCCCCCCFrom settings you can configure buffs, view shopping list, and customize the addon|r")
...
local function OnAddonLoaded()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cff00FF00Akkio's Consume Helper|r |cffFFFF00v" .. ADDON_VERSION ..
